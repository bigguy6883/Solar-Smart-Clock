...
class TouchHandler:
                ...
                if event.type == ecodes.EV_ABS:
                    ...
                    if (
                        ...
                        if self.debug:
                            ...
                            print(
                                f"Touch START: raw=({current_x}, {current_y}) -> screen=({sx}, {sy})",
                                flush=True,
                            )
                    ...
                    if event.value == 1:  # Touch down
                        ...
                        if self.touch_start_x is not None and current_x is not None:
                            if self.debug:
                                sx, sy = self._raw_to_screen(current_x, current_y)
                                print(
                                    f"Touch END: raw=({current_x}, {current_y}) -> screen=({sx}, {sy})",
                                    flush=True,
                                )
                            self._handle_touch(
                                self.touch_start_x,
                            ...
                            if self.debug:
                                print(
                                    "Touch UP ignored (missing coordinates)", flush=True
                                )
                        self.touch_start_x = None
                        self.touch_start_y = None
                        self.touch_start_time = None
    ...
    def _handle_touch(self, start_raw_x, start_raw_y, end_raw_x, end_raw_y):
        ...
        if self.debug:
            print(
                f"Touch UP: screen=({end_x}, {end_y}) delta=({delta_x}, {delta_y}) dur={duration:.2f}s",
                flush=True,
            )
        ...
        if abs(delta_x) > self.swipe_threshold and abs(delta_x) > abs(delta_y):
            if delta_x > 0:
                ...
                if self.debug:
                    print(
                        f"SWIPE RIGHT -> {self.view_manager.get_current()}", flush=True
                    )
            else:
                self.view_manager.next_view()  # Swipe left = next
                if self.debug:
                    print(
                        f"SWIPE LEFT -> {self.view_manager.get_current()}", flush=True
                    )
            return
        ...
        if abs(delta_x) < self.tap_threshold and abs(delta_y) < self.tap_threshold:
            if duration < self.tap_timeout:
                ...
                if self.debug:
                    print(
                        f"Tap too slow ({duration:.2f}s > {self.tap_timeout}s)",
                        flush=True,
                    )
        else:
            if self.debug:
                print(
                    f"Movement too large for tap: ({abs(delta_x)}, {abs(delta_y)})",
                    flush=True,
                )
    ...
    def _handle_tap(self, screen_x, screen_y):
        ...
        if screen_y >= HEIGHT - NAV_BAR_HEIGHT - 10:  # 10px tolerance for edge
            if screen_x < 80:  # Left button area (slightly larger hit area)
                ...
                if self.debug:
                    print(
                        f"TAP left button -> {self.view_manager.get_current()}",
                        flush=True,
                    )
                return
                ...
                if self.debug:
                    print(
                        f"TAP right button -> {self.view_manager.get_current()}",
                        flush=True,
                    )
                return
            else:
            ...
            if self.debug:
                print(
                    f"Tap outside nav bar: y={screen_y} (nav starts at {HEIGHT - NAV_BAR_HEIGHT})",
                    flush=True,
                )
...
class SolarClock:
    ...
    def _load_fonts(self):
        ...
        if font_path:
            ...
            fonts = {
                k: d for k in ["huge", "large", "med", "small", "tiny", "micro", "nav"]
            }
        return fonts
    ...
    def get_sun_times(self):
        try:
            return sun(
                LOCATION.observer, date=datetime.date.today(), tzinfo=LOCATION.timezone
            )
        except:
            return None
    ...
    def get_twilight_times(self):
        ...
        try:
            ...
            result = {}
            civil_dawn, civil_dusk = twilight(
    ...
    def get_golden_hour(self):
        ...
        try:
            ...
            if sunrise:
                ...
                morning = (sunrise, sunrise + datetime.timedelta(minutes=45))

            if sunset:
                # Evening golden hour: ~45 min before sunset to sunset (sun descends through golden zone)
                evening = (sunset - datetime.timedelta(minutes=45), sunset)
    ...
    def get_weather_forecast(self):
        ...
        try:
            ...
            if current_r.status_code == 200 and forecast_r.status_code == 200:
                ...
                self.weather_json = self._convert_openweather_format(
            ...
            "current_condition": [
                {
                    ...
                    "weatherDesc": [
                        {"value": current["weather"][0]["description"].title()}
                    ],
                    "windspeedMiles": str(int(current["wind"]["speed"])),
                    "winddir16Point": self._degrees_to_compass(
                        current["wind"].get("deg", 0)
                    ),
                }
            ],
            "weather": [],
            ...
            daily[date]["hourly"].append(
                {
                    ...
                    "weatherDesc": [
                        {"value": item["weather"][0]["description"].title()}
                    ],
                }
            )
    ...
    def draw_nav_bar(self, draw):
        ...
        draw.rounded_rectangle(
            [(8, btn_y), (8 + btn_width, btn_y + btn_height)],
            radius=6,
        ...
        # Right button
        draw.rounded_rectangle(
            [(WIDTH - 8 - btn_width, btn_y), (WIDTH - 8, btn_y + btn_height)],
            radius=6,
            fill=NAV_BUTTON_COLOR,
        )
        draw.text(
            (WIDTH - 8 - btn_width + 20, btn_y + 2),
            ">",
            fill=WHITE,
            font=self.fonts["nav"],
        )
    ...
    def draw_sun_arc(self, draw, elev, cx, cy, radius):
        ...
        if elev is not None:
            if elev >= 0:
                ...
                draw.ellipse(
                    [(sx - 10, sy - 10), (sx + 10, sy + 10)], fill=(120, 80, 0)
                )
                # Sun body
                draw.ellipse(
                    [(sx - 8, sy - 8), (sx + 8, sy + 8)],
                ...
                draw.ellipse(
                    [(sx - 5, sy - 5), (sx + 5, sy + 5)], fill=PURPLE, outline=GRAY
                )
        ...
        if phase is not None:
            ...
            if phase <= 0.5:
                for y_off in range(-radius, radius + 1):
                    ...
                    if x_inner < x_edge:
                        draw.line(
                            [(cx + x_inner, cy + y_off), (cx + x_edge, cy + y_off)],
                            fill=MOON_YELLOW,
                        )
            else:
                for y_off in range(-radius, radius + 1):
                    ...
                    if x_inner < x_edge:
                        draw.line(
                            [(cx - x_edge, cy + y_off), (cx - x_inner, cy + y_off)],
                            fill=MOON_YELLOW,
                        )
    ...
    def create_clock_frame(self):
        ...
        tw = bbox[2] - bbox[0]
        draw.text(
            (WIDTH // 2 - tw // 2, 5), time_str, fill=WHITE, font=self.fonts["huge"]
        )
        ...
        tw = bbox[2] - bbox[0]
        draw.text(
            (WIDTH // 2 - tw // 2, 58), date_str, fill=WHITE, font=self.fonts["small"]
        )
        ...
        if sun_times:
            ...
            draw.text(
                (145, 94), f"{hours}h {mins}m", fill=WHITE, font=self.fonts["small"]
            )
        ...
        if weather:
            ...
            if len(parts) >= 2:
                ...
                draw.text(
                    (16, 161), condition[:10], fill=cond_color, font=self.fonts["med"]
                )
                draw.text((130, 161), temp, fill=WHITE, font=self.fonts["med"])
                if humidity:
                    draw.text(
                        (185, 166), humidity, fill=LIGHT_BLUE, font=self.fonts["small"]
                    )
        ...
        if bar_width > 3:
            draw.rounded_rectangle(
                [(10, 207), (10 + bar_width, 226)], radius=4, fill=BLUE
            )
        draw.text(
            (85, 232),
            f"{prog*100:.0f}% of day",
            fill=LIGHT_GRAY,
        ...
        # Solar position box
        draw.rounded_rectangle(
            [(235, 88), (WIDTH - 8, 175)], radius=6, fill=(25, 25, 35)
        )
        ...
        if elev is not None:
            ...
            draw.text(
                (245, 112), f"{elev:.1f}°", fill=elev_color, font=self.fonts["med"]
            )
        ...
        if next_event and time_delta:
            ...
            draw.text(
                (235, 240),
                f"{evt_name} in {hours}h {mins}m",
                fill=event_color or ORANGE,
                font=self.fonts["tiny"],
            )
    ...
    def get_next_solar_event(self):
        ...
        try:
            ...
            try:
                tomorrow_sun = sun(
                    LOCATION.observer, date=tomorrow, tzinfo=LOCATION.timezone
                )
    ...
    def create_sunpath_frame(self):
        ...
        draw.rounded_rectangle(
            [(8, chart_box_top), (WIDTH - 8, chart_box_bottom)],
            radius=6,
            fill=(15, 15, 25),
        )
        ...
        for mins in range(0, 24 * 60, 30):
            check_time = datetime.datetime.combine(
                today, datetime.time(0, 0), tzinfo=tz
            )
        ...
        def elev_to_y(e):
            if e >= 0:
                ...
                normalized = (
                    e / min_elev if min_elev < 0 else 0
                )  # 0 to 1 as we go deeper
                y_offset = (
                    day_portion * chart_height
                    + normalized * night_portion * chart_height
                )
        ...
        # Draw zones
        draw.rectangle(
            [(chart_left, astro_y), (chart_right, chart_bottom)], fill=(20, 20, 40)
        )
        draw.rectangle(
            [(chart_left, nautical_y), (chart_right, astro_y)], fill=(30, 30, 55)
        )
        draw.rectangle(
            [(chart_left, civil_y), (chart_right, nautical_y)], fill=(40, 45, 70)
        )
        draw.rectangle(
            [(chart_left, chart_top), (chart_right, civil_y)], fill=(50, 60, 90)
        )
        ...
        for hour in [0, 6, 12, 18, 24]:
            ...
            if hour < 24:
                lbl = f"{hour:02d}"
                draw.text(
                    (x - 6, chart_bottom + 2), lbl, fill=GRAY, font=self.fonts["micro"]
                )
            else:
                draw.text(
                    (x - 6, chart_bottom + 2), "24", fill=GRAY, font=self.fonts["micro"]
                )
        ...
        if elev is not None:
            ...
            # Vertical time line
            draw.line(
                [(x, chart_top), (x, chart_bottom)], fill=(100, 100, 120), width=1
            )
            ...
            # Sun body
            draw.ellipse(
                [(x - 8, y - 8), (x + 8, y + 8)], fill=ORANGE, outline=WHITE, width=2
            )
        ...
        draw.text(
            (10, chart_top - 2), f"{max_elev}°", fill=GRAY, font=self.fonts["micro"]
        )
        mid_elev = max_elev // 2
        ...
        draw.text(
            (10, chart_bottom - 10), f"{min_elev}°", fill=GRAY, font=self.fonts["micro"]
        )
        ...
        # Left box - countdown
        draw.rounded_rectangle(
            [(8, bottom_y), (8 + left_box_w, nav_y - 4)], radius=6, fill=(25, 25, 35)
        )
        ...
        if next_event and time_delta:
            ...
            draw.text(
                (x, bottom_y + 5), str(hours), fill=YELLOW, font=self.fonts["med"]
            )
        ...
        draw.rounded_rectangle(
            [(right_x, bottom_y), (WIDTH - 8, nav_y - 4)], radius=6, fill=(25, 25, 35)
        )

        if elev is not None:
            elev_color = YELLOW if elev > 0 else (ORANGE if elev > -6 else PURPLE)
            draw.text(
                (right_x + 8, bottom_y + 8),
                f"El {elev:.0f}°",
                fill=elev_color,
                font=self.fonts["med"],
            )
            draw.text(
                (right_x + 8, bottom_y + 34),
                f"Az {azim:.0f}°",
                fill=LIGHT_GRAY,
                font=self.fonts["small"],
            )
            ...
            draw.text(
                (right_x + 8, bottom_y + 20), "--", fill=GRAY, font=self.fonts["med"]
            )
    ...
    def draw_weather_icon(self, draw, x, y, size, condition):
        ...
        if "clear" in condition or "sunny" in condition:
            ...
            for dx, dy in [(-6, 0), (6, 0), (0, -6)]:
                draw.text(
                    (x + dx - 4, y + dy - 6), "*", fill=WHITE, font=self.fonts["small"]
                )
            ...
            # Lightning bolt
            draw.polygon(
                [(x, y), (x - 3, y + 8), (x + 2, y + 5), (x - 1, y + 14)], fill=YELLOW
            )
        else:
    ...
    def create_weather_frame(self):
        ...
        if forecast and "current_condition" in forecast:
            ...
            draw.text(
                (18, 108), f"Feels {feels}°", fill=LIGHT_GRAY, font=self.fonts["small"]
            )
            draw.text(
                (115, 108), f"{humidity}%", fill=LIGHT_BLUE, font=self.fonts["small"]
            )
            ...
            draw.text(
                (18, 195),
                f"{wind_speed} mph {wind_dir}",
                fill=WHITE,
                font=self.fonts["med"],
            )

        draw.text(
            (15, 245), f"{LOCATION.name}", fill=LIGHT_GRAY, font=self.fonts["tiny"]
        )
        ...
        if forecast and "weather" in forecast:
            ...
            for i, day in enumerate(forecast["weather"][:3]):
                ...
                if "hourly" in day and len(day["hourly"]) > 0:
                    max_rain = max(
                        int(h.get("chanceofrain", "0")) for h in day["hourly"]
                    )
                ...
                row_y = y + i * 58
                draw.rounded_rectangle(
                    [(x, row_y), (WIDTH - 10, row_y + 52)], radius=6, fill=(20, 20, 30)
                )
                draw.text(
                    (col_day, row_y + 15), day_name, fill=WHITE, font=self.fonts["med"]
                )
                draw.text(
                    (col_high, row_y + 15),
                    f"{max_temp}°",
                ...
                if rain_val > 50:
                    ...
                    draw.ellipse(
                        [(drop_x - 4, row_y + 17), (drop_x + 4, row_y + 25)],
                        fill=LIGHT_BLUE,
                    )
                elif rain_val > 20:
    ...
    def create_moon_frame(self):
        ...
        if moon:
            ...
            draw.rounded_rectangle(
                [(10, top_box_y1), (470, top_box_y2)], radius=8, fill=(20, 20, 30)
            )
            ...
            draw.rounded_rectangle(
                [(10, row1_y1), (10 + box_width, row1_y2)], radius=6, fill=(25, 25, 35)
            )
            draw.text(
                (20, row1_y1 + 6), "Moonrise", fill=GRAY, font=self.fonts["micro"]
            )
            if moon_rise:
                rise_str = moon_rise.strftime("%-I:%M %p")
                draw.text(
                    (20, row1_y1 + 22), rise_str, fill=WHITE, font=self.fonts["small"]
                )
            else:
                draw.text(
                    (20, row1_y1 + 22),
            ...
            # Moonset box
            draw.rounded_rectangle(
                [(10 + box_width + gap, row1_y1), (10 + 2 * box_width + gap, row1_y2)],
                radius=6,
                fill=(25, 25, 35),
            )
            draw.text(
                (20 + box_width + gap, row1_y1 + 6),
                "Moonset",
                fill=GRAY,
            ...
            draw.rounded_rectangle(
                [(cycle_box_x1, row1_y1), (470, row1_y2)], radius=6, fill=(25, 25, 35)
            )
            draw.text(
                (cycle_box_x1 + 10, row1_y1 + 6),
                "Lunar Cycle",
                fill=GRAY,
                font=self.fonts["micro"],
            )
            ...
            draw.rounded_rectangle(
                [
                    (cycle_bar_x, row1_y1 + 24),
                    (cycle_bar_x + cycle_bar_width, row1_y1 + 34),
                ],
                radius=3,
                fill=DARK_GRAY,
            )
            ...
            draw.line(
                [(full_moon_x, row1_y1 + 22), (full_moon_x, row1_y1 + 36)],
                fill=MOON_YELLOW,
                width=2,
            )
            if cycle_fill > 2:
                draw.rounded_rectangle(
                    [
                        (cycle_bar_x, row1_y1 + 24),
                        (cycle_bar_x + cycle_fill, row1_y1 + 34),
                    ],
                    radius=3,
                    fill=PURPLE,
                )
            ...
            # New Moon box
            draw.rounded_rectangle(
                [(10, row2_y1), (10 + half_width, row2_y2)], radius=6, fill=(20, 25, 40)
            )
            draw.text(
                (20, row2_y1 + 6), "New Moon", fill=LIGHT_GRAY, font=self.fonts["tiny"]
            )
            next_new = moon["next_new"]
            days_to_new = (next_new.date() - datetime.date.today()).days
            draw.text(
                (20, row2_y1 + 22),
                next_new.strftime("%b %-d"),
                fill=WHITE,
                font=self.fonts["med"],
            )
            draw.text(
            ...
            # Full Moon box
            draw.rounded_rectangle(
                [(10 + half_width + gap, row2_y1), (470, row2_y2)],
                radius=6,
                fill=(35, 30, 20),
            )
            draw.text(
                (20 + half_width + gap, row2_y1 + 6),
                "Full Moon",
                fill=LIGHT_GRAY,
                font=self.fonts["tiny"],
            )
            ...
            draw.text(
                (20 + half_width + gap, row2_y1 + 22),
                next_full.strftime("%b %-d"),
                fill=WHITE,
                font=self.fonts["med"],
            )
            draw.text(
                (120 + half_width + gap, row2_y1 + 26),
                f"{days_to_full}d",
                fill=MOON_YELLOW,
                font=self.fonts["small"],
            )
    ...
    def create_solar_frame(self):
        ...
        draw.rounded_rectangle(
            [(8, 50), (235, 175)], radius=8, fill=(25, 25, 35), outline=(60, 60, 80)
        )
        ...
        if sun_times:
            ...
            if sunrise and sunset:
                ...
                mins = int((day_len.total_seconds() % 3600) // 60)
                draw.text(
                    (255, 58), "Day Length", fill=LIGHT_GRAY, font=self.fonts["small"]
                )
                draw.text(
                    (255, 80), f"{hours}h {mins}m", fill=WHITE, font=self.fonts["med"]
                )
        ...
        draw.text(
            (18, 190), "Morning Golden Hour", fill=YELLOW, font=self.fonts["small"]
        )
        if morning_gh:
            start_t = morning_gh[0].strftime("%-I:%M")
            end_t = morning_gh[1].strftime("%-I:%M %p")
            draw.text(
                (18, 208), f"{start_t} - {end_t}", fill=ORANGE, font=self.fonts["small"]
            )
        else:
        ...
        # Evening golden hour
        draw.rounded_rectangle(
            [(245, 183), (WIDTH - 8, 230)], radius=6, fill=(40, 30, 25)
        )
        draw.text(
            (255, 190), "Evening Golden Hour", fill=YELLOW, font=self.fonts["small"]
        )
        if evening_gh:
    ...
    def get_air_quality(self):
        ...
        now = time.time()
        if (
            hasattr(self, "aqi_data")
            and self.aqi_data
            and (now - self.aqi_last_update) < 1800
        ):
    ...
    def create_airquality_frame(self):
        ...
        for name, value, max_val, unit in pollutants:
            ...
            val_text = f"{value:.1f}"
            draw.text(
                (bar_x + 5, bar_y + 2), val_text, fill=WHITE, font=self.fonts["small"]
            )
            bar_y += 28
        ...
        draw.text(
            (25, info_y), f"{LOCATION.name}", fill=LIGHT_GRAY, font=self.fonts["small"]
        )
        if aqi == 0:
            draw.text(
                (200, info_y),
    ...
    def create_daylength_frame(self):
        ...
        for day_of_year in range(1, 366):
            try:
                date = datetime.date(year, 1, 1) + datetime.timedelta(
                    days=day_of_year - 1
                )
        ...
        if not day_lengths:
            draw.text(
                (100, 100), "No data available", fill=GRAY, font=self.fonts["med"]
            )
            self.draw_nav_bar(draw)
            return img
        ...
        # Chart background
        draw.rounded_rectangle(
            [(chart_left - 2, chart_top - 2), (chart_right + 2, chart_bottom + 2)],
            radius=4,
            fill=(15, 15, 25),
        )
        ...
        for hours in range(int(min_dl), int(max_dl) + 2, 2):
            if min_dl <= hours <= max_dl:
                ...
                draw.line(
                    [(chart_left, y), (chart_right, y)], fill=(40, 40, 50), width=1
                )
        ...
        for i in range(12):
            ...
            if -half_year <= offset <= half_year:
                x = chart_left + int(
                    (offset + half_year) / (2 * half_year) * chart_width
                )
        ...
        draw.line(
            [(today_x, chart_top), (today_x, chart_bottom)], fill=(60, 60, 100), width=1
        )

        # Draw today marker
        draw.ellipse(
            [(today_x - 7, today_y - 7), (today_x + 7, today_y + 7)], fill=(80, 60, 0)
        )
        draw.ellipse(
        ...
        box1_w = 118
        draw.rounded_rectangle(
            [(8, info_top), (8 + box1_w, info_top + box_h)],
            radius=6,
            fill=(25, 25, 35),
            outline=(60, 60, 80),
        )
        ...
        draw.text(
            (15, info_top + 5), "Today", fill=LIGHT_GRAY, font=self.fonts["small"]
        )
        draw.text(
            (15, info_top + 22), f"{hours}h {mins}m", fill=WHITE, font=self.fonts["med"]
        )
        ...
        if diff_mins > 0.1:
            ...
            trend_color = GRAY
        draw.text(
            (15, info_top + 48), trend_text, fill=trend_color, font=self.fonts["small"]
        )
        ...
        box2_w = 180
        draw.rounded_rectangle(
            [(box2_x, info_top), (box2_x + box2_w, info_top + box_h)],
            radius=6,
            fill=(25, 25, 35),
            outline=(60, 60, 80),
        )
        ...
        col1_x = box2_x + 8
        draw.text(
            (col1_x, info_top + 5), "Shortest", fill=BLUE, font=self.fonts["tiny"]
        )
        draw.text(
            (col1_x, info_top + 20),
            f"{min_hours}h {min_mins}m",
            fill=WHITE,
            font=self.fonts["small"],
        )
        draw.text(
            (col1_x, info_top + 40),
            min_entry[2].strftime("%b %d"),
            fill=GRAY,
            font=self.fonts["tiny"],
        )
        ...
        col2_x = box2_x + 95
        draw.text(
            (col2_x, info_top + 5), "Longest", fill=YELLOW, font=self.fonts["tiny"]
        )
        draw.text(
            (col2_x, info_top + 20),
            f"{max_hours}h {max_mins}m",
            fill=WHITE,
            font=self.fonts["small"],
        )
        draw.text(
            (col2_x, info_top + 40),
            max_entry[2].strftime("%b %d"),
            fill=GRAY,
            font=self.fonts["tiny"],
        )
        ...
        draw.rounded_rectangle(
            [(box3_x, info_top), (WIDTH - 8, info_top + box_h)],
            radius=6,
            fill=(25, 25, 35),
            outline=(60, 60, 80),
        )
        ...
        draw.text(
            (box3_x + 8, info_top + 5), "Next", fill=LIGHT_GRAY, font=self.fonts["tiny"]
        )
        draw.text(
            (box3_x + 8, info_top + 20),
            next_event_name,
            fill=next_event_color,
            font=self.fonts["small"],
        )
        draw.text(
            (box3_x + 8, info_top + 40),
            next_event.strftime("%b %d"),
            fill=WHITE,
            font=self.fonts["tiny"],
        )
        ...
        draw.text(
            (WIDTH - 16 - days_w, info_top + 14),
            days_str,
            fill=next_event_color,
            font=self.fonts["med"],
        )
        draw.text(
            (WIDTH - 16 - days_w, info_top + 42),
            "days",
            fill=GRAY,
            font=self.fonts["tiny"],
        )
    ...
    def create_analemma_frame(self):
        ...
        if not EPHEM_AVAILABLE:
            draw.text(
                (100, 150), "ephem library required", fill=GRAY, font=self.fonts["med"]
            )
            self.draw_nav_bar(draw)
            return img
        ...
        draw.text(
            (chart_cx + 5, 252), "winter", fill=LIGHT_BLUE, font=self.fonts["micro"]
        )
        ...
        if today_eot is not None:
            ...
            draw.ellipse(
                [(today_x - 8, today_y - 8), (today_x + 8, today_y + 8)],
                fill=(80, 60, 0),
            )
            draw.ellipse(
                [(today_x - 6, today_y - 6), (today_x + 6, today_y + 6)], fill=WHITE
            )

        # Right panel - today's info (expanded height)
        info_x = 308
        draw.rounded_rectangle(
            [(info_x, 48), (WIDTH - 8, 272)],
            radius=8,
            fill=(25, 25, 35),
            outline=(60, 60, 80),
        )
        ...
        if today_eot is not None:
            ...
            draw.text(
                (info_x + 10, 105),
                f"{abs(today_eot):.1f} min",
                fill=timing_color,
                font=self.fonts["large"],
            )
            draw.text(
                (info_x + 10, 138), timing, fill=timing_color, font=self.fonts["med"]
            )
            ...
            draw.text(
                (info_x + 10, 176), "Sun path", fill=GRAY, font=self.fonts["small"]
            )
            draw.text(
                (info_x + 10, 196), height, fill=height_color, font=self.fonts["large"]
            )
            draw.text(
                (info_x + 10, 228),
                f"{abs(today_decl):.1f}° {['S','N'][today_decl > 0]}",
                fill=GRAY,
                font=self.fonts["med"],
            )
    ...
    def create_analogclock_frame(self):
        ...
        for i in range(12):
            ...
            if i == 0:
                ...
                draw.ellipse(
                    [(dx - r_inner, dy - r_inner), (dx + r_inner, dy + r_inner)],
                    fill=colors["face"],
                )
            elif i % 3 == 0:
                # 3, 6, 9: larger dots
